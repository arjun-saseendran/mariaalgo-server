...
const { ApiClient, WebsocketApi } = pkg;
...
export const initUpstoxLiveData = async () => {
  ...
  // ── Connect via Upstox JS SDK WebSocket ────────────────────────────────────
  // The Upstox JS SDK WebsocketApi gives us a raw WebSocket-compatible object.
  // We set the auth token on the singleton ApiClient first.
  const defaultClient = ApiClient.instance;
  defaultClient.authentications['OAUTH2'].accessToken = token;

  const wsApi = new WebsocketApi();

  // getPortfolioStreamFeed / getMarketDataFeed are the two WS endpoints.
  // getMarketDataFeed = market quotes (what we need).
  wsApi.getMarketDataFeedAuthorize(
    process.env.UPSTOX_API_VERSION || '2.0',
    async (error, data) => {
      if (error) {
        console.error('❌ Upstox WS auth failed:', error);
        return;
      }

      // data.data.authorizedRedirectUri contains the WSS URL
      const wsUrl = data?.data?.authorizedRedirectUri;
      if (!wsUrl) {
        console.error('❌ Upstox WS: no authorized URL returned');
        return;
      }

      // Use native WebSocket (Node 18+ has it globally, else use 'ws' package)
